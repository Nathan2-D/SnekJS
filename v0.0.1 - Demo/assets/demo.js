if (window.CSS && CSS.number) {
  ...
  window.addEventListener('scroll', (e) => { snekTypedOM('.container','.s-snek')  }, { capture: false, passive: true})
}else{
  window.alert('You are using CSSOM version, switch to Google Chrome +66, to see it in CSS Typed OM')
  window.addEventListener('scroll', (e) => { snek('.container','.s-snek')  }, { capture: false, passive: true})
}

// let didScroll = false
// window.onscroll = doScroll
// function doScroll() {didScroll = true}

// setInterval(function() {
//     if(didScroll) {
//         didScroll = false;
//         snekTypedOM('.container','.s-snek')
//     }
// }, 1);
//
let w = window,
...
const triggerLine = ((windowHeight/100) * 95)
//
d.getElementById('toTop').onclick = function(e){
  d.getElementsByTagName('body')[0].scrollIntoView({
    behavior : 'smooth',
    block : 'start'
  })
}
...
function snekTypedOM(parent, container){
  ...
  if(typeof activeElement !== 'undefined'){
    ...
    if(activeElement.classList.contains('active') && typeof span1 !== 'undefined'  && typeof span2 !== 'undefined'){
      ...
      // eg treatment
      else if(activeElement.classList.contains('s-eg')){
        ...
        if(relativeScroll < elementHalf){
          ...
          eg1_rS.innerHTML = Math.round(relativeScroll)
          eg1_eH.innerHTML = elementHalf
          eg1_r.innerHTML = (Math.round((relativeScroll / elementHalf) * 100))
          span1.attributeStyleMap.set('width', CSS.percent((relativeScroll / elementHalf) * 100))
        ...
        if(relativeScroll > elementHalf){
          ...
          eg2_rS.innerHTML = Math.round(relativeScroll)
          eg2_eH.innerHTML = (elementHalf * 2)
          eg2_r.innerHTML = (Math.round(((relativeScroll - (elementHalf)) / (elementHalf)) * 100))
          span2.attributeStyleMap.set('height', CSS.percent(((relativeScroll - (elementHalf)) / (elementHalf)) * 100))
        }else{
          span1.attributeStyleMap.set('height', CSS.percent(0))
