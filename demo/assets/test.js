...
function snekTypedOM(parent, container, trigger = 50){
  ...
  if(typeof triggerLine == 'undefined'){
    triggerLine = ((windowHeight/100) * trigger)
    console.log('[-3]triggerLine set')
  }
  fromTop = (window.pageYOffset || dE.scrollTop)  - (dE.clientTop || 0)
  // generate array containing each snek elems  // (once only)
  if(!arrayReady){
    console.log('[-2]arrayGen')
  ...
  // & maybe even add two more span in some case, this will offer more possibilities for cool effects on the background
  if (!snekInside) {
    console.log('[-1]snekGen')
    snekInside = true
    for (let i = snekContainers.length - 1; i >= 0; i--) {
      for(j = 1; j >= 0; j--){
  ...
  if(sC.length == snekContainers.length && !elementIsEligible){
    ...
    if(eActive.length>0){
      // console.log('[1]found '+eActive.length+' to activate')
      hasTargets = true
    }
  }
  ...
  if(eActive.length > 0 && hasTargets){
    if(activeElement.offsetTop < (triggerLine + fromTop)  && Math.round(activeElement.offsetTop + activeElement.clientHeight) > (triggerLine + fromTop)){
      // console.log('[2]activatedTargetisEligible')
      elementIsEligible = true
      for (index in eActive) {
        if(!eActive[index].hasData){
...
function treatment(activeElement){
  let logic
  // here we find the logic we need to apply based on the css class of element we just receive
  ...
  !logic? logic = 'default' : false
  // console.log('[3]weTreat-->'+logic)
  switch(logic) {
